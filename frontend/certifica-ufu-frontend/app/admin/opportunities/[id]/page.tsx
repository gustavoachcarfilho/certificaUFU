"use client"

import React, { useState, useEffect } from 'react';
import { useRouter, useParams } from 'next/navigation'
import Link from 'next/link'
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'
import { Textarea } from '@/components/ui/textarea'
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card'
import { useToast } from '@/hooks/use-toast'
import { ArrowLeft, Save, Calendar as CalendarIcon, Clock, Loader2 } from 'lucide-react'
import { getOpportunityById, updateOpportunity } from '@/lib/api'

import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle,
} from "@/components/ui/dialog"

/**
 * Main component for editing an existing opportunity.
 * Replaces mocks with real project dependencies.
 */
export default function App() {
  const [title, setTitle] = useState('');
  const [description, setDescription] = useState('');
  const [hours, setHours] = useState('');
  
  // Date strings in YYYY-MM-DD format for HTML5 date inputs
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  
  const [isLoading, setIsLoading] = useState(true);
  const [isUpdating, setIsUpdating] = useState(false);
  const [isConfirmOpen, setIsConfirmOpen] = useState(false);
  
  const router = useRouter();
  const params = useParams();
  const id = params?.id as string;
  const { toast } = useToast();

  // Fetch current opportunity data from the backend on mount
  useEffect(() => {
    const fetchOpportunity = async () => {
      if (!id) return;
      
      try {
        const data = await getOpportunityById(id);
        setTitle(data.title);
        setDescription(data.description);
        setHours(data.hours.toString());
        setStartDate(data.startDate);
        setEndDate(data.endDate);
      } catch (error) {
        toast({
          title: "Erro ao carregar dados",
          description: "Não foi possível encontrar esta oportunidade.",
          variant: "destructive",
        });
        router.push('/admin/dashboard');
      } finally {
        setIsLoading(false);
      }
    };

    fetchOpportunity();
  }, [id, router, toast]);

  // Client-side validation before showing the confirmation dialog
  const validateForm = () => {
    if (!title.trim() || !description.trim() || !hours || !startDate || !endDate) {
      toast({
        title: "Campos obrigatórios",
        description: "Por favor, preencha todas as informações.",
        variant: "destructive",
      });
      return false;
    }

    if (new Date(startDate) > new Date(endDate)) {
      toast({
        title: "Erro no cronograma",
        description: "A data de início não pode ser posterior à data de término.",
        variant: "destructive",
      });
      return false;
    }

    return true;
  };

  const handleSubmitTrigger = (e: React.FormEvent) => {
    e.preventDefault();
    if (!validateForm()) return;
    setIsConfirmOpen(true);
  };

  // Final execution of the update API call
  const handleConfirmUpdate = async () => {
    setIsConfirmOpen(false);
    setIsUpdating(true);

    const opportunityData = {
      title,
      description,
      hours: Number(hours),
      startDate,
      endDate,
    };

    try {
      await updateOpportunity(id, opportunityData);
      toast({
        title: "Oportunidade Atualizada!",
        description: "As alterações foram salvas com sucesso.",
      });
      router.push('/admin/dashboard');
    } catch (error: any) {
      toast({
        title: "Falha na atualização",
        description: error.message || "Ocorreu um erro ao salvar as alterações.",
        variant: "destructive",
      });
    } finally {
      setIsUpdating(false);
    }
  };

  if (isLoading) {
    return (
      <div className="flex h-screen items-center justify-center bg-gray-50 dark:bg-gray-900">
        <Loader2 className="w-8 h-8 animate-spin text-primary" />
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <header className="bg-white dark:bg-gray-800 shadow-sm border-b">
        <div className="container mx-auto py-4 px-4 sm:px-6">
          <div className="flex items-center space-x-4">
            <Link href="/admin/dashboard">
              <Button variant="ghost" size="sm">
                <ArrowLeft className="w-4 h-4 mr-2" />
                Voltar
              </Button>
            </Link>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">
              Editar Oportunidade
            </h1>
          </div>
        </div>
      </header>

      <main className="container mx-auto py-6 sm:py-8 px-4 sm:px-6">
        <div className="max-w-2xl mx-auto">
          <Card className="shadow-md border-none">
            <CardHeader>
              <CardTitle>Atualizar Detalhes</CardTitle>
              <CardDescription>
                Modifique as informações abaixo para atualizar a atividade complementar no sistema.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmitTrigger} className="space-y-6">
                <div className="space-y-2">
                  <Label htmlFor="title">Título da Oportunidade *</Label>
                  <Input 
                    id="title" 
                    value={title} 
                    onChange={(e) => setTitle(e.target.value)} 
                    required 
                  />
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="description">Descrição Detalhada *</Label>
                  <Textarea 
                    id="description" 
                    value={description} 
                    onChange={(e) => setDescription(e.target.value)} 
                    className="min-h-[120px]"
                    required 
                  />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="hours">Horas Totais *</Label>
                    <Input 
                      id="hours" 
                      type="number" 
                      value={hours} 
                      onChange={(e) => setHours(e.target.value)} 
                      required 
                    />
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="startDate">Início *</Label>
                    <Input 
                      id="startDate" 
                      type="date" 
                      value={startDate} 
                      onChange={(e) => setStartDate(e.target.value)} 
                      required 
                    />
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="endDate">Término *</Label>
                    <Input 
                      id="endDate" 
                      type="date" 
                      value={endDate} 
                      onChange={(e) => setEndDate(e.target.value)} 
                      required 
                    />
                  </div>
                </div>

                <div className="flex pt-4">
                  <Button type="submit" className="w-full text-base font-semibold py-6" disabled={isUpdating}>
                    {isUpdating ? (
                      <span className="flex items-center">
                        <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                        Salvando...
                      </span>
                    ) : (
                      <span className="flex items-center">
                        <Save className="w-4 h-4 mr-2" />
                        Salvar Alterações
                      </span>
                    )}
                  </Button>
                </div>
              </form>
            </CardContent>
          </Card>
        </div>
      </main>

      {/* Final Confirmation Modal */}
      <Dialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
        <DialogContent className="sm:max-w-[440px] rounded-xl">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold">Confirmar Alterações</DialogTitle>
            <DialogDescription className="text-sm pt-2">
              Você está prestes a atualizar esta oportunidade. Todos os candidatos visualizarão os dados novos imediatamente.
            </DialogDescription>
          </DialogHeader>
          
          <div className="bg-gray-50 dark:bg-gray-900/50 p-4 rounded-lg space-y-3 my-2 border border-gray-100 dark:border-gray-800">
            <div className="flex flex-col border-b border-gray-200 dark:border-gray-800 pb-2">
              <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Título</span>
              <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{title}</span>
            </div>
            
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col">
                <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Carga Horária</span>
                <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{hours} horas</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Período</span>
                <div className="flex flex-col text-[11px] font-medium text-gray-700 dark:text-gray-300">
                  <span>De: {startDate ? new Date(startDate).toLocaleDateString('pt-BR') : ''}</span>
                  <span>Até: {endDate ? new Date(endDate).toLocaleDateString('pt-BR') : ''}</span>
                </div>
              </div>
            </div>
          </div>

          <DialogFooter className="flex sm:justify-between gap-3 mt-4">
            <Button 
              variant="outline" 
              onClick={() => setIsConfirmOpen(false)} 
              className="flex-1"
              disabled={isUpdating}
            >
              Cancelar
            </Button>
            <Button 
              onClick={handleConfirmUpdate} 
              disabled={isUpdating}
              className="flex-1 bg-primary hover:bg-primary/90 shadow-sm"
            >
              {isUpdating ? 'Salvando...' : 'Confirmar Atualização'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
}